...
对应 [Spec](spec.md)。基线 `9ff9f94`；状态：待派发。任务打勾必须有对应提交与证据，派发不算实现完成。
...
- [ ] P-01 提交 Spec/tasks/索引，记录共同起点；创建两个新工作树，保留各任务旧工作树。
- [ ] P-02 向两任务派发职责、绝对路径、限制、验收和回传方式；不覆盖现有模型设置。
- [ ] P-03 确认两任务收到并开始工作；在本表记录状态/阻塞，不重复调取长历史。
