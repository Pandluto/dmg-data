...
type AppAction =
  ...
  | {
      ...
      nodeIndex?: number;
      nodeNumber?: number;
      releaseAnchor?: SkillButton['releaseAnchor'];
    }
  ...
  | {
      ...
      buttonId: string;
      bundle: BasicAttackTailBundle | null;
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'REMOVE_SKILL_BUTTON': {
      const removed = state.skillButtons.find(button => button.id === action.buttonId);
      const bundle = removed?.basicAttackTailBundle;
      const peerId = bundle
        ? bundle.predecessorButtonId === action.buttonId
          ? bundle.successorButtonId
          : bundle.predecessorButtonId
        : null;
      return {
        ...state,
        ...
        skillButtons: state.skillButtons.map((btn) =>
          btn.id === action.buttonId
            ? {
                ...
                ...(action.releaseAnchor !== undefined && { releaseAnchor: action.releaseAnchor }),
              }
            : btn
        ),
      };
    ...
    case 'SET_BASIC_ATTACK_TAIL_BUNDLE': {
      return {
        ...state,
        skillButtons: state.skillButtons.map((button) => {
          if (button.id !== action.buttonId) return button;
          const next = { ...button };
          if (action.bundle) next.basicAttackTailBundle = action.bundle;
          else delete next.basicAttackTailBundle;
          if (action.stageCount !== null) next.basicAttackStageCount = action.stageCount;
          else delete next.basicAttackStageCount;
          return next;
        }),
      };
    }
