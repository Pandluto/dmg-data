...
状态：2026-09-12 实现、有限验收和主任务审查完成，本次合并交付主仓。对应 [Spec](spec.md) 与 [tasks](tasks.md)。
...
小队 runner 只有原基线约 L1186 的实际扣费调用与事件上下文 hunk（现约 L1186–1215）。不涉及排序、队列、控制权、释放依赖；与逻辑序改造整合时保留其合同，只接入这段事务。其余改动在 runtime、单人/替代入口、assembler、compiler、状态机及来源映射。实施工作树由原测试任务独占，主任务负责本次主仓合并。

## 主任务复核与整合

主任务逐项审查扣费四类入口、原始身份优先的冷却装配、独立层时钟与自然到期/消费/清除处理，以及测试的实际 import 与断言。原始三个最小 fixture 与新 fixture 字节一致；13 项代码、来源和 fixture 指纹全部匹配交付记录。

合并前主仓相对于施工基线只有派发状态文档变化，业务代码自动合并。冲突仅在 tasks 的开始/完成状态及派发记录，保留派发基线并更新为完成。整合后的实现、脚本、测试和输入与已验收分支一致；因此只补文档结构/链接、差异空白和指纹检查，没有重复运行六个定向检查或三份完整输入。原始审计工作树与证据仍保留。

本次确认三个缺陷已修复，不代表全部武器、满层替换规则或未实现语义已覆盖；保留上文已说明的限制。无推送、部署或在线服务操作。
