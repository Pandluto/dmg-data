...
import { installAkeCatalogData } from '../../integrations/ake/akeCatalogAdapter';
...
export function WebBootstrap() {
  ...
  const initializeWorkspace = useCallback(async () => {
    ...
    try {
      ...
      if (isAkeDemo) {
        if (!hasAnyAppliedIndependentLibraries()) {
          const response = await fetch('/data/default-local-data.json', { cache: 'no-store' });
          if (!response.ok) {
            throw new Error(`无法载入 LTS 本地数据包：HTTP ${response.status}`);
          }
          const saved = await saveLocalDataPackage({
            scope: 'share',
            archive: await response.json(),
            sourceName: 'ake-demo-bundled-lts-data',
            replace: true,
          });
          await applyLocalDataPackage({
            scope: 'share',
            packageId: saved.summary.packageId,
            backup: false,
          });
        }
        await installAkeCatalogData();
