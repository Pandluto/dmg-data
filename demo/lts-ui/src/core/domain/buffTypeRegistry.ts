...
export type BuffTypeMatchRule =
  ...
  | { kind: 'elements'; elements: Array<'fire' | 'electric' | 'ice' | 'nature'> }
  | { kind: 'physical' }
  | { kind: 'magic' }
...
const entries: BuffTypeRegistryEntry[] = [
  ...
  { type: 'fireNatureDmgBonus', zone: 'damageBonus', match: { kind: 'elements', elements: ['fire', 'nature'] }, allowMultiplier: true, valueStyle: 'ratio' },
  { type: 'iceElectricDmgBonus', zone: 'damageBonus', match: { kind: 'elements', elements: ['ice', 'electric'] }, allowMultiplier: true, valueStyle: 'ratio' },
