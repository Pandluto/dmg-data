...
对应 [Spec](spec.md)。基线 `9ff9f94`；共同规格提交 `11ef693b867b83fb733db9db69c186eb8500a702`；状态：2026-09-12 已完成本地主库接入和本轮窄验收，原未覆盖项继续保留。任务打勾必须有对应提交与证据，派发不算实现完成。

**当前交付：** 用户追加授权将 `codex/ui-0912` 的 `26564db` 合入含机制修复的 main `8e8e7b8`；已在隔离工作树完成手动协调与窄验收，见 [主库合并记录](implementation-main-merge.md)。B 的实现为 `b4a82f8` / `6e65c9d`，A 与主任务的保存、预演及顺序实现为 `b71e537` / `2a0ae61`。原 `43822` 预览仍保持在原工作树版本，历史进度及未覆盖项见 [预览集成记录](implementation-integration.md)。
...
- [x] I-06 用户追加授权下将 `26564db` 与主库 `8e8e7b8` 手动合并；保留扣费、冷却和叠层修复，以既有归档协调洛茜／卡缪日志哈希，补齐两条按钮表读取纯度路径，完成有限回归、类型检查、构建和离线接线。证据见 [主库合并记录](implementation-main-merge.md)，不覆盖 I-03 至 I-05 的全部未验收内容。
