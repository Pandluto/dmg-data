...
export function doesBuffTypeMatchHit(
  type: string | undefined,
  context: BuffHitContext,
): boolean {
  if (!type) return false;
  if (type === 'multiplierMultiplier') return true;

  const entry = getBuffTypeRegistryEntry(type);
  if (entry) return matchesRule(entry.match, context);

  // 面板属性会作用于每个伤害 Hit，但不属于五个伤害乘区注册表。
  if (PANEL_HIT_BUFF_TYPES.has(type)) return true;

  if (type === 'allCorrosion' || type === 'allResistanceIgnore') return true;
  if (type === 'physicalCorrosion' || type === 'physicalResistanceIgnore') {
    return context.element === 'physical';
  }
  if (type === 'magicCorrosion' || type === 'magicResistanceIgnore') {
    return context.element !== 'physical';
  }
  for (const element of ['fire', 'electric', 'ice', 'nature'] as const) {
    if (type === `${element}Corrosion` || type === `${element}ResistanceIgnore`) {
      return context.element === element;
    }
  }

  return type === 'comboDamageBonus' || type === 'imbalanceDmgBonus';
}

const PANEL_HIT_BUFF_TYPES = new Set([
  'atkPercentBoost',
  'flatAtk',
  'mainStat',
  'subStat',
...
function matchesRule(rule: BuffTypeMatchRule, context: BuffHitContext): boolean {
  switch (rule.kind) {
    ...
    case 'elements':
      return rule.elements.includes(context.element as 'fire' | 'electric' | 'ice' | 'nature');
    case 'skillType':
