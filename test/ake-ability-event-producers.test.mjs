...
test('public BuffData ability events are measured against concrete runtime producers', () => {
    ...
    assert.equal(publicProducerEntries.length, 22);
    assert.equal(publicProducerEntries.reduce((sum, [, count]) => sum + count, 0), 613);
...
test('top-level Buff listeners fail closed on missing or malformed event producers', () => {
    ...
    const fixture = {
        ...
        abilityEventAction: [
            removeAllGroup,
            { ...removeAllGroup, abilityEvent: 'OnOwnerHpZero' },
...
test('Skill passive listener groups use the same producer audit', () => {
    ...
    assert.ok(passive.groups.some(group =>
        group.eventType === 'OnConsumeBuff'
        && group.unresolved.some(gap =>
            gap.code === 'AKE_ABILITY_EVENT_EMITTER_REQUIRED'
        )
    ));
