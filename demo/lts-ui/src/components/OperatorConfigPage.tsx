...
function createDefaultCharacterConfig(character: Character): OperatorConfigPageCharacterConfig {
  const emptyWeapon = createEmptyWeaponState();
  return {
    character: {
      id: character.id,
      config: {
    ...
    weapon: {
      ...emptyWeapon,
      data: character.defaultWeaponId
        ? { akeDefaultWeaponId: character.defaultWeaponId }
        : emptyWeapon.data,
    },
    equipment: {
...
function hydrateDraftConfigFromLibraries(
  ...
  const currentWeaponData = config.weapon.data as Partial<WeaponData> & { akeDefaultWeaponId?: string };
  const pendingDefaultWeaponId = currentWeaponData.akeDefaultWeaponId || '';
  const weaponData = weaponLibrary[weaponName]
    ?? (!weaponName && pendingDefaultWeaponId
      ? Object.values(weaponLibrary).find(weapon => weapon.id === pendingDefaultWeaponId)
      : undefined);
...
function formatWeaponSkillValue(
  skillKey: 'skill1' | 'skill2',
  skill: WeaponSkillData,
  levelData: WeaponSkillLevelData | undefined
): string {
  if (typeof levelData?.value === 'number') {
    if (skillKey === 'skill2' && skill.statType !== '源石技艺提升' && Math.abs(levelData.value) <= 1) {
      return `${Number((levelData.value * 100).toFixed(2))}%`;
    }
...
function formatWeaponSkillSummary(skillKey: 'skill1' | 'skill2', skill: WeaponSkillData | undefined, level: number): { label: string; typeKey: string; tail: string; full: string } {
  ...
  if (!skill.statType?.trim() && typeof levelData?.value !== 'number') {
    const label = skill.name?.trim() || '无第二词条';
    return { label, typeKey: '', tail: '', full: label };
  }
  ...
  return getWeaponSummaryParts(chineseLabel, englishType || '-', `+ ${formatWeaponSkillValue(skillKey, skill, levelData)}`);
}
...
export function OperatorConfigPage() {
  ...
  const [akeDefaultWeaponByCharacterId, setAkeDefaultWeaponByCharacterId] = React.useState<Record<string, string>>({});
  ...
  React.useEffect(() => {
    ...
    void loadAkeCatalog()
      .then(catalog => setAkeDefaultWeaponByCharacterId(Object.fromEntries(
        catalog.characters.map(character => [character.id, character.defaultWeaponId])
      )))
      .catch(error => console.error('[OperatorConfigPage] 读取 AKE 默认武器失败', error));
  }, []);
  ...
  const ensureCharacterConfig = React.useCallback(
    (characterId: string, sourceCharacter?: Character | null) => {
      ...
      setConfigMap((prev) => {
        ...
        let nextCharacterConfig = existing
          ? {
            ...existing,
            character: {
              ...existing.character,
        ...
        const currentWeaponData = nextCharacterConfig.weapon.data as { akeDefaultWeaponId?: string; akeDefaultOptOut?: boolean };
        const defaultWeaponId = currentWeaponData.akeDefaultWeaponId
          || character.defaultWeaponId
          || akeDefaultWeaponByCharacterId[character.id]
          || '';
        const defaultWeapon = !nextCharacterConfig.weapon.id && !currentWeaponData.akeDefaultOptOut && defaultWeaponId
          ? Object.values(weaponLibrary).find(weapon => weapon.id === defaultWeaponId)
          : undefined;
        if (defaultWeapon) {
          nextCharacterConfig = {
            ...nextCharacterConfig,
            weapon: {
              ...nextCharacterConfig.weapon,
              id: defaultWeapon.name,
              data: defaultWeapon as unknown as Record<string, unknown>,
            },
          };
        }
    ...
    [akeDefaultWeaponByCharacterId, getCharacterById, weaponLibrary]
  );
