...
/**
 * A button's combat start is a relationship, not a pixel-derived timestamp.
 * `group-start` has no source; every other kind creates one directed edge from
 * the source action to this button.  Damage anchors retain the observed offset
 * so a later profile refresh can diagnose a changed hit instead of silently
 * moving the release point.
 */
export type SkillReleaseAnchorKind =
  | 'group-start'
  | 'action-start'
  | 'action-end'
  | 'damage-hit';
 ...
 * stable if a runtime catalog uses a different tick rate.
 */
export type ForcedWaitConfig =
  | {
    schemaVersion: 1;
    mode: 'seal-only';
  }
...
/**
 * A confirmed early-cut normal attack and the action snapped behind it form
 * one immutable UI bundle. Both buttons persist the same reference so reload,
 * snapshot restore and deletion can repair the pair without relying on pixels.
 */
export interface BasicAttackTailBundle {
  id: string;
...
/** AKE 原始命中动作中与 DamageAction 同事件组触发的效果。 */
export interface HitBuffEffect {
  id: string;
  displayName: string;
  target: string;
  targetLabel?: string;
  kind?: 'status' | 'resource' | 'attachment' | string;
  /** AKE 引擎状态的稳定标识，避免依赖本地化名称猜测。 */
  statusKey?: string;
  /** 状态自身的绝对量，例如本次命中的失衡值。 */
  statusValue?: number;
...
export interface SandboxSkill {
  ...
  /** Present only for entries from the fifth, global interaction toolbox. */
...
export interface OperatorBuffActivation {
...
export interface OperatorBuffEffect {
  ...
  activation?: OperatorBuffActivation;
}
...
export interface Character {
  ...
  /** AKE 目录提供的初始武器；仅用于新建配置时预选，不覆盖用户清空。 */
...
export interface SkillButton {
  ...
  isLocked?: boolean;
  /** 已确认执行的普攻段数；删除 bundle 尾部动作后自动清除。 */
  basicAttackStageCount?: number;
  /** 普攻截段与后继动作的原子拼接关系。 */
  basicAttackTailBundle?: BasicAttackTailBundle;
  /** Authoritative release dependency. Pixels/nodeIndex remain presentation hints. */
  releaseAnchor?: SkillReleaseAnchor;
  /** Fifth-toolbox control node; never treat it as damage skill data. */
  timelineModuleKind?: TimelineModuleKind;
...
export interface SkillButtonData {
  ...
  basicAttackStageCount?: number; // 已确认执行的普攻段数
  basicAttackTailBundle?: BasicAttackTailBundle; // 与后继动作的锁定拼接关系
