...
function calculateMobileSlot(
  ...
  const isImbalanced = (action.anomalyStatuses ?? []).some((card) => card.key === 'imbalance-state');
  const configuredHitBuffs = buildConfiguredHitBuffs(snapshot, {
    isImbalanced,
    statuses: (action.anomalyStatuses ?? []).map((card) => card.key),
    physicalAnomalyTriggered: (action.anomalyDamages ?? [])
      .some((card) => card.category === 'physical'),
  });
  const configuredModifierBuffs = applyMobileSourceFilter(
    action.id,
    configuredHitBuffs.modifierBuffs,
    sourceFilter,
    sourceSidecar,
  );
  const configuredDisplayOnlyBuffs = applyMobileSourceFilter(
    action.id,
    configuredHitBuffs.displayOnlyBuffs,
    sourceFilter,
    sourceSidecar,
  );
  const configuredExtraHitBuffs = applyMobileSourceFilter(
    action.id,
    configuredHitBuffs.extraHitBuffs,
    sourceFilter,
    sourceSidecar,
  ).filter((buff): buff is SkillButtonBuff & {
    effectKind: 'extraHit';
    extraHitConfig: NonNullable<SkillButtonBuff['extraHitConfig']>;
  } => buff.effectKind === 'extraHit' && Boolean(buff.extraHitConfig));
  const combinedModifierBuffs = [
    ...modifierBuffs,
    ...stateDerivedBuffs,
    ...anomalyStateBuffs,
    ...configuredModifierBuffs,
  ];
  ...
  const result = calculateSkillButtonDamageV2({
    ...
    displayOnlyBuffs: configuredDisplayOnlyBuffs,
    ...
    targetState: { isImbalanced, armorBreakLevel: 0, attachments: [] },
  });
  const specialSegments = buildAnomalyDamageSegments({
    ...
    damageBonus: effectiveDamageBonus,
    targetResistance: action.targetResistance,
    targetState: { isImbalanced },
    fullCombinedModifierBuffList: combinedModifierBuffs,
    extraHitBuffList: [...extraHitBuffs, ...configuredExtraHitBuffs],
    buffStackCounts: action.buffStackCounts,
