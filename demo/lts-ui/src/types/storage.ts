...
export interface SkillButtonBuff {
  ...
  /**
   * 仅用于逐 Hit 归因展示；该效果已经结算进配置面板，或本身只是状态标记，
   * 因此不能再次进入面板/乘区计算。
   */
  displayOnly?: boolean;
...
export interface PersistedSkillButton {
  ...
  basicAttackTailBundle?: BasicAttackTailBundle; // 普攻截段与后继动作的原子 bundle
  releaseAnchor?: SkillReleaseAnchor;   // 真实起手关系；nodeIndex 只保留显示/迁移用途
