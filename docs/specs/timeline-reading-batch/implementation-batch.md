...
- `SkillSandbox` 在阅读入口旁增加批量 SVG 入口；批量状态与阅读状态互斥。
- `useBatchTimelineSelection` 只在画布上接管 pointer 事件。`batchSelectionGeometry` 从可见操作 DOM 的技能圆心、等待轨道或切人轨道取中心，状态角标不会成为独立选择目标。
