...
interface BuildAnomalyDamageSegmentsParams {
  ...
  targetState?: { isImbalanced?: boolean };
...
function doesBuffApplyToSpecialHit(
  buff: SkillButtonBuff,
  element: SpecialHitElement,
  skillType: SpecialHitSkillType,
): boolean {
  if (buff.type === 'sourceSkillBoost') return true;
  if (!buff.type) return false;
  const entry = getBuffTypeRegistryEntry(buff.type);
  if (element === 'magic') {
    if (entry?.match.kind === 'element' || entry?.match.kind === 'elements' || entry?.match.kind === 'physical') {
      return false;
    }
    if (/^(?:fire|electric|ice|nature)(?:Corrosion|ResistanceIgnore)$/.test(buff.type)) {
      return false;
    }
  }
  if (!skillType && (entry?.match.kind === 'skillType' || entry?.match.kind === 'skillTypes')) {
    return false;
  }
  return doesBuffTypeMatchHit(buff.type, {
    element: element === 'magic' ? 'nature' : element,
    skillType: skillType || 'A',
  });
}
...
export function buildAnomalyDamageSegments({
  ...
  const anomalySegments = selectedAnomalyDamages.flatMap<AnomalyDamageSegmentView>((card) => {
    ...
    const imbalanceDamageBonus = buffTotals.imbalanceDamageBonus + (
      elementKey === 'physical' && (targetState?.isImbalanced ?? true)
  ...
  const extraHitSegments = extraHitBuffList.flatMap<AnomalyDamageSegmentView>((buff) => {
    ...
    const segments = Array.from({ length: hitCount }).flatMap<AnomalyDamageSegmentView>((_, hitIndex) => {
      ...
      const combinedAppliedBuffs = fullCombinedModifierBuffList.filter((item) => (
        doesBuffApplyToSpecialHit(item, elementKey, extraHitConfig.skillType)
