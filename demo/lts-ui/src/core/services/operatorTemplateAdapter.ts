...
function normalizeBuffEffect(effectKey: string, rawEffect: unknown): OperatorDraftBuffEffect {
  ...
  const rawActivation = source.activation && typeof source.activation === 'object'
    ? source.activation as Record<string, unknown>
    : null;
  const activationKind = typeof rawActivation?.kind === 'string'
    && ['targetImbalanced', 'targetStatus', 'targetAttachment', 'targetHpBelow'].includes(rawActivation.kind)
    ? rawActivation.kind as NonNullable<OperatorDraftBuffEffect['activation']>['kind']
    : null;
  return {
    schemaVersion: 2,
    ...
    ...(activationKind ? {
      activation: {
        kind: activationKind,
        ...(typeof rawActivation?.status === 'string' ? { status: rawActivation.status } : {}),
        ...(typeof rawActivation?.element === 'string' ? { element: rawActivation.element } : {}),
        ...(typeof rawActivation?.value === 'number' && Number.isFinite(rawActivation.value)
          ? { value: rawActivation.value }
          : {}),
      },
    } : {}),
...
export function buildRuntimeOperatorTemplateFromDraft(
  ...
  return {
    ...
    defaultWeaponId: normalizedDraft.defaultWeaponId || undefined,
